...
class _FakeTorch:
    ...
    class nn:
        ...
        class functional:
            ...
            @staticmethod
            def normalize(value):  # noqa: ANN001, ARG004
                return value
...
class _FakeTensorBatch:
    ...
    def unsqueeze(self):  # noqa: ARG002
        return self
...
class _FakeTokenizerLoader:
    ...
    @staticmethod
    def from_pretrained(**kwargs):  # noqa: ANN002, ANN003
...
class TopicPrefilterTests(unittest.TestCase):
    ...
    def test_build_topic_matcher_returns_disabled_matcher_when_prefilter_is_off(self) -> None:
        matcher = build_topic_matcher(_config(topic_prefilter_enabled=False))

        self.assertIsInstance(matcher, BaseTopicMatcher)
        self.assertFalse(matcher.enabled)
        self.assertIsNone(matcher.score_paper(_paper()))
    ...
    def test_fake_runtime_helpers_cover_loader_and_tensor_branches(self) -> None:
        ...
        self.assertIs(batch.unsqueeze(), batch)
        ...
        self.assertIsInstance(_FakeTokenizerLoader.from_pretrained(trust_remote_code=True), _FakeTokenizer)
        self.assertIsInstance(_FakeModelLoader.from_pretrained(trust_remote_code=True), _FakeModel)
        with self.assertRaisesRegex(AssertionError, "trust_remote_code"):
            _FakeTokenizerLoader.from_pretrained()
        with self.assertRaisesRegex(AssertionError, "trust_remote_code"):
            _FakeModelLoader.from_pretrained()
    ...
    def test_local_topic_matcher_scores_high_relevance_and_tracks_used_sections(self) -> None:
        ...
        paper = _paper(raw_payload={"keywords": ["AI governance", "deployment"], "full_text_excerpt": "Detailed governance analysis for healthcare AI."})
    ...
    def test_default_topic_prefilter_model_uses_bge_small(self) -> None:
        config = _config(topic_prefilter_enabled=True)
    ...
    def test_local_topic_matcher_can_auto_filter_low_relevance(self) -> None:
        config = _config(topic_prefilter_enabled=True, topic_prefilter_filter_low_relevance=True)

        with patch("analysis.topic_prefilter.load_embedding_runtime", return_value=(_FakeTorch, _FakeTokenizerLoader, _FakeModelLoader)), \
                patch.object(LocalTopicMatcher, "_embed_texts", return_value=[_FakeVector(1.0), _FakeVector(0.20)]):
            matcher = LocalTopicMatcher(config)
            result = matcher.score_paper(_paper(title="Clinical biomarkers for oncology", abstract="Purely medical biomarker study."))
    ...
    def test_keyword_match_details_skip_empty_rules_and_can_end_as_weak_fit(self) -> None:
        ...
        self.assertEqual(_weighted_keyword_score([]), 0.0)
    ...
    def test_ai_screener_uses_topic_prefilter_to_exclude_low_relevance_papers(self) -> None:
        config = _config(topic_prefilter_enabled=True, topic_prefilter_filter_low_relevance=True, llm_provider="heuristic")
        paper = _paper(title="Medical imaging biomarkers", abstract="A medical imaging paper without AI governance content.")
